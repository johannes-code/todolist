...
export default function TodoListComponent() {
  ...
  const fetchWithAuth = async (url, options = {}) => {
     if (!isSignedIn || !session) {
         ...
         const authError = new Error("Bruker ikke logget inn eller sesjon ikke klar.");
         (authError as any).statusCode = 401; // Legger til en statuskode for enkelhet
         throw authError;
     }
     ...
     if (!response.ok) {
         ...
         const backendError = new Error(`Backend feil: ${response.status} ${response.statusText} - ${errorText}`);
         (backendError as any).statusCode = response.status;
         throw backendError;
     }
  ...
  async function fetchTodos() { // Fjern 'token' parameter, hentes internt
    setLoading(true);
    ...
    try {
      ...
      setError(`Kunne ikke laste todos: ${error instanceof Error ? error.message : String(error)}`);
  ...
  async function initializeEncryptionKey() {
    ...
    if (!isSignedIn || !session || !userId) {
        console.log("Skipping key initialization: Not signed in, session not ready, or user ID missing.");
        return;
    }
     setError(null); // Nullstill feil
    ...
    try {
      ...
      const userProfileResponse = await fetchWithAuth(`/api/user-profile/${userId}`);
      ...
      if (!userProfileData.hasEncryptedKey) {
        ...
        const storeKeyResponse = await fetchWithAuth("/api/store-encryption-key", {
          method: "POST",
          body: JSON.stringify(keyStoragePayload),
        });

        if (storeKeyResponse.ok) { // Sjekk ok status her også, selv om fetchWithAuth kaster feil på !ok
        ...
        if (userProfileDataWithKey && userProfileDataWithKey.encryptedKey) {
          try {
             ...
             const keyArrayBuffer = new Uint8Array(userProfileDataWithKey.encryptedKey).buffer;
            ...
            console.error("Error importing existing key:", error);
             setError(`Feil ved import av nøkkel: ${error instanceof Error ? error.message : String(error)}`);
          }
        } else {
      ...
      // Sett feilmelding i state
      setError(`Feil under nøkkelinitialisering: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  return (
    <>
        ...
        <div className="container mx-auto p-4">
            ...
            {isSignedIn ? (
                <>
                    <h1 className="text-4xl font-bold mb-6 text-white">Todo App {user ? `for ${user.firstName || user.emailAddresses?.[0].emailAddress}` : ''}</h1>
                    ...
                    {encryptionKey ? (
                        ...
                        />
                         ...
                         <p className="text-yellow-500">Initialiserer krypteringsnøkkel...</p>
                    )}

                    <div className="mt-6 space-y-2">
                        {/* Itererer over todos og sender hver todo til TodoItem */}
                        {todos.length === 0 && !loading && !error && <p className="text-gray-400">Ingen todos funnet. Legg til noen!</p>}
