...
export async function POST(request) {
  ...
  // Få tak i den autentiserte brukerens ID fra Clerk
   const { userId: authenticatedUserId } = auth();
    ...
    if (!authenticatedUserId) {
      console.log("API - POST /store-encryption-key - Unauthorized: User not logged in");
  ...
  try {
    ...
    console.log("API - /api/store-encryption-key - requestedUserId:", requestedUserId); // Logg den forespurte ID'en fra body
    console.log("API - /api/store-encryption-key - exportedKey (preview):", exportedKey ? exportedKey.slice(0, 10) + '...' : null); // Logg en forkortet versjon
     ...
     if (authenticatedUserId !== requestedUserId) {
        console.warn(`API - POST /store-encryption-key - User ID mismatch: Authenticated ${authenticatedUserId}, Requested ${requestedUserId}`);
    ...
    if (!exportedKey) { // Vi har allerede sjekket authenticatedUserId
    ...
    console.log("API - /api/store-encryption-key - Connected to collection: users");

    // Finn brukeren ved hjelp av den AUTENTISERTE bruker-ID-en
    const existingUser = await usersCollection.findOne({ userId: authenticatedUserId });
    console.log("API - /api/store-encryption-key - Existing user found with authenticatedId:", existingUser ? true : false);
    ...
    if (!existingUser) {
      ...
      console.log(`API - /api/store-encryption-key - No existing user found for authenticatedId ${authenticatedUserId}. Creating new document.`);
      ...
      console.log(`API - /api/store-encryption-key - Encryption key stored for new user: ${authenticatedUserId}`);
      ...
      console.log(`API - /api/store-encryption-key - Found existing user for authenticatedId ${authenticatedUserId}. Updating key.`);
      ...
      console.log(`API - /api/store-encryption-key - Encryption key updated for existing user: ${authenticatedUserId}`);
