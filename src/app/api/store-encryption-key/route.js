...
export async function POST(request) {
  ...
  try {
    ...
    if (!userId || !exportedKey) {
      console.log(
        "API - /api/store-encryption-key - Missing userId or exportedKey"
      ); // Added log
    ...
    console.log(
      "API - /api/store-encryption-key - Existing user:",
      existingUser
    ); // Added log
    ...
    if (!existingUser) {
      ...
      console.log(
        `API - /api/store-encryption-key - Encryption key stored for new user: ${userId}`
      );
      return NextResponse.json(
      ...
      console.log(
        `API - /api/store-encryption-key - Encryption key updated for existing user: ${userId}`
      );
      return NextResponse.json(
